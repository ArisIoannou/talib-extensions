...
def HHLL(high, low, periods=20):
    ...
    return np.hstack(
        (highestHighVector.reshape((highestHighVector.size, 1)), lowestLowVector.reshape((lowestLowVector.size, 1)),
         midPointVector.reshape((midPointVector.size, 1))))


def CMF(high, low, close, volume, periods=20):
    vectorSize = high.shape[0];
    moneyFlowMultiplier = ((close - low) - (high - close)).reshape((vectorSize, 1)) * np.linalg.pinv(
        (high - low).reshape((vectorSize, 1)));
